...
public class EditGiaoDich extends JFrame {
...
//					frame.setVisible(true);
	...
	public void showdata(){
		...
		try {
			...
			ResultSet rs0 = ManagerDatabase.getGiaoDich(maBDS);
			rs0.next();
			tfMa.setText(maBDS);
			...
			System.out.println(rs0.getString(7) + "Gia tri tra ve");
			if(rs0.getString(7).equals("Ban"))
				 cbBanMua.setSelectedItem("Ban");
			else if(rs0.getString(7).equals("Thue"))
				 cbBanMua.setSelectedItem("Thue");
			...
			while(rs.next()){
				...
				if(rs.getString(1).equals(rs0.getString(2)))
					cbNhanvien.setSelectedIndex(listNhanVien.size()-1);
			}
			...
			cbNam.setSelectedItem(rs0.getString(3));
			
			for(int i= 1; i < 13; i++ ){
				cbThang.addItem(i<10?("0"+i):i + "");
			}
			cbThang.setSelectedItem(rs0.getString(4));
			
			updateDate(Integer.parseInt(rs0.getString(3)),Integer.parseInt(rs0.getString(4))-1, Integer.parseInt(rs0.getString(5)));
			
//			System.out.printf("%d-%02d-%02d %02d:%02d:%02d.%03d", year, month, day, hour, minute, second, millis);
			
			tfGia.setText(rs0.getString(6));
			rs.close();
	 ...
	 * Create the frame.
	 */
	public EditGiaoDich(String maBDS) {
		this.maBDS = maBDS;
		...
		btnThucHien = new JButton("Update");
		...
		btnThucHien.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				String result = ManagerDatabase.UpdateGiaoDich(tfMa.getText(),listNhanVien.get(cbNhanvien.getSelectedIndex()),
				...
				if(result.equals("success")){					
					System.out.println(ManagerDatabase.executeSqlKinhDoanh("commit"));
