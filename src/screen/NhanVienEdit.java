...
public class NhanVienEdit extends JFrame {
...
//			INSERT INTO SYS.NHANVIEN values (sys.seq_MaNhanVien.nextval,'Nguyen Van A', '11-11-1990', '113496780','nguyenvananh@gmai.com', '0922345678', '2015', 'Truong Chi Nhanh',700000, 'D0001', 'P0001', '', 1 );
			...
			rs.next();
			if(rs.getInt(1) != 0 && rs.getString(1).equals(nhanvien.getCMND())){
			...
			String sqlUpdate =
					"Update SYS.NHANVIEN set Hoten="+ Hoten + ", ngaysinh=" + NgaySinh+ ", cmnd=" + CMND  + ", email=" +Email+ ", sdt=" + Phone+  ", namvao="+ NamVao + ", chucvu=" + Chucvu +", luong=" + Luong + ",maduong=" +  Duong + ", maphuong=" + Phuong + ", macn = " +  ChiNhanh + " where manv=" +nhanvien.getMaNV();
			
			System.out.println(sqlUpdate);
			String returnMess = ManagerDatabase.executeSql(sqlUpdate);
	...
	void createDataForComboBox(){
		...
		try {
			...
			tfMa.setText(nhanvien.getMaNV());
			tfName.setText(nhanvien.getHoTen());
			tfNgaySinh.setText(nhanvien.getNgaySinh());;
			tfCMND.setText(nhanvien.getCMND());;
			tfLuong.setText(nhanvien.getLuong());
			...
			while(rs.next()){
				...
				if(rs.getString(1).equals(nhanvien.getMaTP()))
					cbThanhPho.setSelectedItem(listThanhPho.size()-1);
			}
			
		
			//list quan
			rs = ManagerDatabase.getTableQuan(nhanvien.getMaTP());
			...
			while(rs.next()){
				...
				if(rs.getString(1).equals(nhanvien.getMaQuan()))
					cbQuan.setSelectedIndex(listQuan.size() - 1);
			}
					
			//list Phuong
			rs = ManagerDatabase.getTablePhuong(nhanvien.getMaQuan());
			...
			while(rs.next()){
				...
				if(rs.getString(1).equals(nhanvien.getMaphuong()))
			...
			while(rs.next()){
				...
				if(rs.getString(1).equals(nhanvien.getMaduong()))
			...
			while(rs.next()){
				...
				if(rs.getString(1).equals(nhanvien.getMaCN()))
					cbChiNhanh.setSelectedIndex(listChiNhanh.size() - 1);
			}
	 ...
	 * Create the frame.
	 */
	public NhanVienEdit(NhanVien nhanvien) {
		this.nhanvien = nhanvien;
		setTitle("EDIT NH\u00C2N VI\u00CAN");
		...
		JLabel T = new JLabel("M\u00E3");
		T.setBounds(10, 29, 57, 14);
		contentPane.add(T);
